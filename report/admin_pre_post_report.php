...
FROM fb_settings F LEFT JOIN `fb_spend_revenue` R ON R.`Account ID` IN (F.fb_ad_account_id)  LEFT JOIN accounts A ON A.sha_token = F.sha_token WHERE A.sha_token = "'.$_GET['shop'].'" AND F.fb_ad_account_id IS NOT NULL
...
$sql = 'SELECT fb_ad_account_id FROM popsixle.fb_ad_account_id_map WHERE sha_token = "'.$_GET['shop'].'";';
      ...
      <div class="card bg-light mb-2">
        ...
        <div class="p-4">
          <div class="">		
			...
			<div class="card bg-light mb-2">
        <div class="p-4">
					<table style="border-top: none;border-spacing: 2px; border-collapse: separate;">
						<tbody>
							<?php
							$table_data_count = 1;
							foreach ($date_array as $date_key => $date_value) {
								$date_exists = 0;
								foreach ($db_data_daily_check as $db_data_daily_check_key => $db_data_daily_check_value) {
									if ($date_value == $db_data_daily_check_value["Day"]){
										$date_exists = 1;
									}
								}
								if ($date_exists == 1){
									echo '<td data-toggle="tooltip" data-placement="top" data-title="'.date_format(date_create($date_value), "m-d-Y").'" style="text-align:center;color:green" class="date_true">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
								} else {
									echo '<td data-toggle="tooltip" data-placement="top" data-title="'.date_format(date_create($date_value), "m-d-Y").'" style="text-align:center;color:red" class="date_false">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
								}
							}
							echo '<h2 class="order"style="color:#AC39FD">Data Check</h2>';
							?>
						</tbody>
					</table>
				</div>
